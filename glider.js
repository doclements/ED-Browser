...
var csv_sync = require('csv-parse/lib/sync');
...
const front_file = 'data/553_glider/coords_uk_2016-09-12_to_2016-09-18_v0.2.csv';
...
let getFrontLines = function(){
    const front_file = 'data/glider_553/coords_uk_2016-09-12_to_2016-09-18_v0.2.csv';
    
    let contents = fs.readFileSync(front_file);
    
    var data = csv_sync(contents, {columns: true});
    ...
    for (var x = 0; x < data.length; x++) {
        //console.log("testing "+data[x]['Line_id']);
        if (parseInt(data[x]['Line_id']) === line_id){
           temp_holder.push(data[x]);
        }
        else {
          fronts.push(temp_holder);
          temp_holder = []
          line_id = parseInt(data[x]['Line_id']);
          temp_holder.push(data[x]);
    ...
    fronts = fronts.splice(1);
    traces = []
    for (var x = 0; x < 1; x++) {
       var element = fronts[x];
       console.log('######');
       ...
       for (var c = 0; c < element.length; c++) {
        ...
        //  console.log("------------");
          console.log(c);
        //  console.log(item);
         trace.x.push(item.Longitude);
         trace.y.push(item.Latitude);
         trace.z.push(10);
         trace.line.color.push(item.Strength);
         trace.text.push(item.Strength);
       }
       traces.push(trace);
...
csv.fromPath("data/glider_553/553_glider.csv")
      ...
      var data = [
        {
          ...
          z:getLatLon('data/glider_553/gebco.nc'),
          type: "surface"
        }
      ];
      var front_traces = getFrontLines();
      fs.writeFileSync("trace1.json", JSON.stringify(trace1));
      fs.writeFileSync("trace_bathy.json", JSON.stringify(trace_bathy));
      fs.writeFileSync("trace_overlay.json", JSON.stringify(trace_overlay));
      fs.writeFileSync("trace_fronts.json", JSON.stringify(...front_traces));
       
    //   var graphOptions = {layout: layout, filename: "merge mess 3", fileopt: "overwrite"};
    //   plotly.plot([trace1,trace_bathy,trace_overlay,...front_traces], graphOptions, function (err, msg) {
